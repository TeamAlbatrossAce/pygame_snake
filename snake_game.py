...
class Snake(object):
    ...
    def draw(self, screen):
        red, green, blue = 100 / (self.length - 1), 200, 200 / (self.length - 1)
        for i, p in enumerate(self.positions):
            color = (self.color + red * i, green, blue * i)
...
class Game(object):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    ...
                    self.snake.changeColor()
        return False

    # 게임 로직 수행
    def run_logic(self):
        self.snake.move()
        self.check_eat(self.snake, self.feed1, self.obstacle)
        self.check_eat(self.snake, self.feed2, self.obstacle)
        self.check_crash(self.snake, self.obstacle, self.feed1)
        self.check_crash(self.snake, self.obstacle, self.feed2)
    ...
    def check_eat(self, snake, feed,obstacle):
        if snake.positions[0] == feed.position:
            ...
            feed.create()
            obstacle.create()
            sound = pygame.mixer.Sound("nowWe.wav")
            sound.play()
            sleep(0.5)
            sound.stop()
    #뱀이 장애물에 충돌했는지 체크    
    def check_crash(self, snake, obstacle,feed):
